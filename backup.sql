...
CREATE POLICY "user_roles_select_policy" ON "public"."user_roles" FOR SELECT TO "authenticated" USING (("user_id" = "auth"."uid"()));
...
CREATE POLICY "ユーザーロールは管理者のみが管理可能" ON "public"."user_roles" USING ((EXISTS ( SELECT 1
   FROM ("public"."user_roles" "ur"
     JOIN "public"."roles" "r" ON (("ur"."role_id" = "r"."id")))
  WHERE (("ur"."user_id" = "auth"."uid"()) AND ("r"."name" = 'admin'::"text")))));
...
CREATE POLICY "ユーザーロール作成ポリシー" ON "public"."user_roles" FOR INSERT TO "authenticated" WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "ユーザーロール参照ポリシー" ON "public"."user_roles" FOR SELECT TO "authenticated" USING (true);
