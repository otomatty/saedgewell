...
export class KeywordResolver {
  ...
  /**
   * コンストラクタ
   * @param documentMapper ドキュメントマッパー
   * @param config 設定
   */
  constructor(documentMapper: DocumentMapper, config: KeywordResolverConfig) {
    ...
    this.contextMatcher = new ContextAwareMatcher();
  ...
  async resolveKeyword(
    ...
    try {
      ...
      try {
        // ドキュメントマッパーでキーワードを検索
        const result = await this.documentMapper.resolveKeyword(
          keyword,
          docType
        );

        const documents = result.mapping
          ? [result.mapping, ...(result.alternatives || [])]
          : [];
        ...
        return {
          ...
          error:
            result.error ||
            `キーワード "${keyword}" に一致するドキュメントが見つかりませんでした。`,
        };
      } catch (error) {
        // エラーを報告して詳細情報を返す
